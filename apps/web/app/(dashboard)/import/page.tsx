  ...
  async function parseFile(f: File) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.error || "Error al parsear archivo");
      }
    } catch {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500">Carga un archivo Excel o CSV con datos de conductores, buses o viajes</p>
      </div>
      ...
      {loading && (
        <div className="text-center py-8 text-gray-500">Procesando archivo...</div>
      )}
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <div className="p-4 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Esta es una vista previa. La importación completa se implementará en una versión futura.
            </p>
          </div>
          <div className="overflow-x-auto">
                ...
                {preview.rows.slice(0, 10).map((row: any[], i: number) => (
                  <tr key={i}>
                    {row.map((cell: any, j: number) => (
                      <td key={j} className="px-3 py-2 text-sm text-gray-600">{cell}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
            ...
            {preview.rows.length > 10 && (
              <p className="text-center text-sm text-gray-500 py-2">Mostrando 10 de {preview.rows.length} filas</p>
            )}
          </div>
        </div>
      )}
